...
namespace Bikiran.Validation
{
    public class ValFormat
    {
        public static StatusObj IsValidDateFormat(string? date, string title = "Date")
        {
            ...
            if (!st)
            {
                return new StatusObj { Error = true, Message = "Please enter valid " + title };
            }
        ...
        public static StatusObj IsValidEmailFormat(string? email, string title, string postText = "")
        {
            ...
            {
                return new StatusObj { Error = true, Message = $"Please enter {title}. {postText}" };
            }

            if (email.Length < 6)
            {
                return new StatusObj { Error = true, Message = $"{title} should be minimum 3 characters long. {postText}" };
            }

            if (email.Length > 200)
            {
                return new StatusObj { Error = true, Message = $" {title} should be maximum 200 characters long. {postText}" };
            }
            ...
            if (!st)
            {
                return new StatusObj { Error = true, Message = $"Please enter valid {title}. {postText}" };
            }

            return new StatusObj { Error = false, Message = "Success" };
        }

        public static StatusObj IsValidEmailFormatAll(List<string> email, string title)
        {
        ...
        public static StatusObj IsValidPasswordFormat(string? password, string title = "Password")
        {
            ...
            {
                return new StatusObj { Error = true, Message = title + " should be minimum 8 characters long" };
            }

            if (password.Length > 32)
            {
                return new StatusObj { Error = true, Message = title + " should be maximum 32 characters long" };
            }
            ...
            if (!st)
            {
                return new StatusObj { Error = true, Message = title + " should contain at least one uppercase letter, one lowercase letter, one digit and one special character" };
            }

            return new StatusObj { Error = false, Message = "Success" };
        }

        public static StatusObj IsValidString(string? name, string title, int min = 2, int max = 100)
        {
            ...
            if (name.Length < min)
            {
                return new StatusObj { Error = true, Message = title + " should be minimum " + min + " character long" };
            }

            if (name.Length > max)
            {
                return new StatusObj { Error = true, Message = title + " should be maximum " + max + " character long" };
            }
            ...
            if (!st)
            {
                return new StatusObj { Error = true, Message = title + " should be alphanumeric" };
            }

            return new StatusObj { Error = false, Message = "Success" };
        }

        public static StatusObj IsValidLongString(string? name, string title, int min = 2, int max = 5000, bool isOptional = false)
        {
        ...
        public static StatusObj IsValidDomainFormat(string? domainName, string title, bool isOptional = false)
        {
            ...
            return new StatusObj { Error = false, Message = title + " is Valid" };
        }

        public static StatusObj IsValidDomainFormatAll(List<string>? domainName, string title, bool isOptional = false)
        {
        ...
        public static StatusObj IsValidUrlFormat(string? logoUrl, string title, bool isOptional = false)
        {
        ...
        public static StatusObj IsValidUserNameFormat(string userName, string title)
        {
            ...
            {
                return new StatusObj { Error = true, Message = title + " should be minimum 5 characters long" };
            }

            if (userName.Length > 20)
            {
                return new StatusObj { Error = true, Message = title + " should be maximum 20 characters long" };
            }
            ...
            if (!st)
            {
                return new StatusObj { Error = true, Message = title + " should be alphanumeric or Dot(.), Hyphen(-), At the rate(@)" };
            }

            //--Check if username not starts with a letter
            if (!char.IsLetter(userName[0]))
            {
                return new StatusObj { Error = true, Message = title + " should start with a letter" };
            }

            //--Check if username not ends with a letter or number
            if (!char.IsLetterOrDigit(userName[^1]))
            {
                return new StatusObj { Error = true, Message = title + " should end with a letter or number" };
            }
        ...
        public static StatusObj IsValidIpFormat(string? ip, string title)
        {
        ...
        public static StatusObj IsValidIpFormatAll(List<string>? ips, string title)
        {
        ...
        public static StatusObj IsValidPathPattern(string sourceDir, string title)
        {
